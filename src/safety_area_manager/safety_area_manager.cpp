...
private:
  ...
  std::optional<mrs_lib::Prism>                   transformPrism(const mrs_lib::Prism& prism, const std::string& target_frame);
  std::tuple<bool, std::vector<mrs_lib::Point2d>> transformPoints(const std::vector<mrs_lib::Point2d>& points, const std::string& from_frame,
...
bool SafetyAreaManager::callbackSetObstacle(mrs_msgs::SetObstacleSrv::Request& req, mrs_msgs::SetObstacleSrv::Response& res) {
  ...
  if (!is_initialized_) {
    res.message = "not initialized";
    res.success = false;
    ROS_WARN("Service request not possible, not initialized");
    return true;
  }

  std::scoped_lock lock(mutex_safety_area_);

  auto prism_ptr = makePrism(req.points, req.max_z, req.min_z, req.horizontal_frame, req.vertical_frame);

  if (!prism_ptr) {
    ROS_WARN("Could not create prism from the provided message");
    res.message = "Could not create prism from the provided message";
    res.success = false;
    return true;
  }

  int id = safety_zone_->addObstacle(std::move(prism_ptr));
...
bool SafetyAreaManager::callbackSaveWorldConfig(mrs_msgs::String::Request& req, mrs_msgs::String::Response& res) {
  ...
  mrs_lib::YamlExportVisitor visitor(_uav_name_, safety_area_horizontal_frame_, safety_area_horizontal_frame_, safety_area_vertical_frame_, world_origin_units_,
                                     origin_x_, origin_y_, transformer_);
...
bool SafetyAreaManager::initializationFromFile(mrs_lib::ParamLoader& param_loader, const std::string& filename) {
  ...
  auto border = makePrism(border_points, max_z, min_z, safety_area_horizontal_frame_, safety_area_vertical_frame_);
  ...
  if (obstacles_present_) {
    ...
    for (size_t i = 0; i < obstacles_mat.size(); i++) {
      ...
      auto       prism     = makePrism(obstacles_mat[i], obs_max_z, obs_min_z, safety_area_horizontal_frame_, safety_area_vertical_frame_);
...
bool SafetyAreaManager::initializationFromMsg(const mrs_msgs::SafetyArea& safety_area_msg) {
  ...
  auto       border = makePrism(border_points, max_z, min_z, safety_border.horizontal_frame, safety_border.vertical_frame);
  ...
  if (obstacles_present_) {
    ...
    for (const auto& obstacle : obstacles) {
      auto prism = makePrism(obstacle.data, max_z, min_z, safety_border.horizontal_frame, safety_border.vertical_frame);
...
std::unique_ptr<mrs_lib::Prism> SafetyAreaManager::makePrism(const Eigen::MatrixXd matrix, const double max_z, const double min_z,
  ...
  return std::make_unique<mrs_lib::Prism>(points, max_z, min_z, horizontal_frame, vertical_frame);
}
...
std::unique_ptr<mrs_lib::Prism> SafetyAreaManager::makePrism(const std::vector<mrs_msgs::Point2D>& points, const double max_z, const double min_z,
  ...
  std::unique_ptr<mrs_lib::Prism> prism;
  try {
    prism = std::make_unique<mrs_lib::Prism>(tmp_points, max_z, min_z, horizontal_frame, vertical_frame);
  }
  catch (const std::exception& e) {
    ROS_WARN("[SafetyAreaManager]: Failed to create prism from message: %s", e.what());
    return nullptr;
  }
...
std::optional<mrs_lib::Prism> SafetyAreaManager::transformPrism(const mrs_lib::Prism& prism, const std::string& target_frame) {
  ...
  double transformed_max_z = transformZ(prism.getVerticalFrame(), target_frame, prism.getMaxZ());
  double transformed_min_z = transformZ(prism.getVerticalFrame(), target_frame, prism.getMinZ());

  if (!success || transformed_max_z == 0 || transformed_min_z == 0) {
    ROS_WARN_THROTTLE(1.0, "Failed to transform safety zone points to %s frame. Will retry later.", target_frame.c_str());
    return std::nullopt;
  }

  return mrs_lib::Prism(transformed_border_points, transformed_max_z, transformed_min_z, target_frame, target_frame);
}
...
std::tuple<bool, std::vector<mrs_lib::Point2d>> SafetyAreaManager::transformPoints(const std::vector<mrs_lib::Point2d>& points, const std::string& from_frame,
                                                                                   const std::string& to_frame) {
  ...
  for (const auto& point : points) {
    ...
    if (auto ret = transformer_->transformSingle(temp_ref, to_frame)) {
      ...
      return std::make_tuple(false, std::vector<mrs_lib::Point2d>{});
    }
  }
...
void SafetyAreaManager::getSafetyZoneData(void) {
  ...
  std::scoped_lock lock(mutex_safety_area_);
  {
    ...
    // only attempt the transform to latlon_origin if we are receiving GPS data
    if (sh_gnss_.hasMsg()) {
      ...
      // Fallback for local-only flights
      origin_x_ = origin_x;
      origin_y_ = origin_y;
    }


    auto [success, transformed_border_points] = transformPoints(border_points, "world_origin", safety_area_horizontal_frame_);
    ...
    for (const auto& [key, obstaclePtr] : obstacles_ptrs) {
      ...
      auto [success, transformed_obstacle] = transformPoints(obstacle, "world_origin", safety_area_horizontal_frame_);
...
std::tuple<bool, bool> SafetyAreaManager::isPositionValid(mrs_msgs::UavState uav_state) {
  ...
  ROS_INFO_STREAM_ONCE("[SafetyAreaManager]: Current position x:  " << current_position.reference.position.x << " y: " << current_position.reference.position.y
